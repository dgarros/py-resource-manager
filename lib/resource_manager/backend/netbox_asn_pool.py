...
class NetboxAsnPool(object):
    ...
    def __init__(self, netbox, name, scope, asn_range=[], custom_field='ASN', secure=True):
        ...
        if not isinstance(asn_range, list):
            ...
            raise Exception('asn_range must be list of 2 items, not %s items' % len(asn_range) )
        
        for item in scope:
            if not isinstance(item, dict):
                raise Exception('scope must be a list of dictionnay, at least 1 item is a %s' % type(item))
        ...
        self.pool = IntegerPool(self.name, 
                                start=asn_range[0], 
                                end=asn_range[1]
                            )
        ...
        for dev in resp['results']:
            ...
            self.pool.reserve( integer=dev['custom_fields'][custom_field], identifier=dev['name'] )
