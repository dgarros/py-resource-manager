...
class Test_NetboxAsnPool(unittest.TestCase):
    ...
    def test_init(self, m):
        ...
        with pytest.raises(Exception) as e_info:
            pool = NetboxAsnPool(
                netbox="http://mock", name="test_range", asn_range=[65001, 65100]
            )

        with pytest.raises(Exception) as e_info:
            pool = NetboxAsnPool(
                netbox="http://mock", name="test_range", scope=[{"site": "test"}]
            )
        ...
        with pytest.raises(Exception) as e_info:
            pool = NetboxAsnPool(
                netbox="http://mock", scope=[{"site": "test"}], asn_range=[65001, 65100]
            )
    ...
    def test_user_defined_custom_field(self, m):
        ...
        m.get(
            "http://mock/api/dcim/devices/?%s" % test03_1["params"],
