...
VALID_CONFIG_2 = {
    "netbox": {"address": "http://mock"},
    "netbox_asn": {"custom_field_name": "newname"},
}
...
class Test_NetboxAsnManager(unittest.TestCase):
    def test_init_not_valid(self):
        ...
        with pytest.raises(Exception) as e_info:
            asnManager = NetboxAsnManager(config={})
    ...
    def test_basic(self, m):
        ...
        asn_manager = NetboxAsnManager(config=VALID_CONFIG_1)
        asn_manager.add_pool_specification(
            name="test_range",
            spec={"scope": [{"site": "test"}], "range": [65001, 65100]},
        )

        self.assertEqual(
            asn_manager.resolve(var_type="ASN", var_params="test_range"), 65003
        )
        self.assertEqual(
            asn_manager.resolve(
                var_type="ASN", var_params="test_range", identifier="device1"
            ),
            65001,
        )
        self.assertEqual(
            asn_manager.resolve(
                var_type="ASN", var_params="test_range", identifier="device10"
            ),
            65010,
        )
